...
(deftest ^:parallel apply-after.correct-time
  (let [done (async/chan)]
    ...
    (is (match (await-completion done 20) :closed :ok)
...
(deftest ^:parallel apply-after.fn-exit
  (let [done (async/chan)]
    ...
    (is (match (await-completion done 70) :closed :ok)
...
(deftest ^:parallel cancel.apply-after
  (let [done (async/chan)
        tref (timer/apply-after 50 #(async/close! done))]
    (timer/cancel tref)
    (is (thrown? Exception (await-completion done 100))
...
(deftest ^:parallel cancel.send-after
  (proc-util/execute-proc!!
    (let [tref (timer/send-after 50 (process/self) :msg)]
      (timer/cancel tref)
      (process/receive!
        :msg (is false "message must not be sent after timer has been canceled")
        (after 100 :ok))))
  (proc-util/execute-proc!!
    (let [tref (timer/send-after 50 :msg)]
      (timer/cancel tref)
      (process/receive!
        :msg (is false "message must not be sent after timer has been canceled")
        (after 100 :ok)))))
...
(deftest ^:parallel cancel.exit-after
  (proc-util/execute-proc!!
    ...
    (let [tref (timer/exit-after 50 (process/self) :abnormal)]
      (timer/cancel tref)
      (process/receive!
        [:EXIT _ :abnormal]
  ...
  (proc-util/execute-proc!!
    (process/flag :trap-exit true)
    (let [tref (timer/exit-after 50 :abnormal)]
      (timer/cancel tref)
      (process/receive!
        [:EXIT _ :abnormal]
        (is false "exit must not be sent after timer has been canceled")
...
(deftest ^:parallel cancel.kill-after
  ...
  (proc-util/execute-proc!!
    (process/flag :trap-exit true)
    (let [tref (timer/kill-after 50)]
      (timer/cancel tref)
      (is (process/receive!  _ false (after 100 :ok))
          "process must not be killed after timer has been canceled"))))

(deftest ^:parallel cancel.apply-interval
  (proc-util/execute-proc!!
    (let [done (async/chan)
          tref (timer/apply-interval 50 async/put! [done :msg])]
      (timer/cancel tref)
  ...
  (proc-util/execute-proc!!
    (let [done (async/chan)
          tref (timer/apply-interval 50 #(async/put! done :msg))]
      (is (= (await-completion done 70) [:ok :msg])
...
(deftest ^:parallel cancel.send-interval
  (proc-util/execute-proc!!
    (let [tref (timer/send-interval 50 :msg)]
      (timer/cancel tref)
      (process/receive!
        :msg
        (is false
...
(deftest ^:parallel cancel.after-timer-finished
  (proc-util/execute-proc!!
    (let [done (async/chan)
      ...
      (is (= :closed (await-completion done 20))
        "fn must be applied just after timeout")
  ...
  (proc-util/execute-proc!!
    ...
    (let [tref (timer/exit-after 0 (process/self) :normal)]
      (process/receive!
        [:EXIT _ :normal] :ok
        (after 20 (is false "exit must be sent just after timeout")))
      (is (do (timer/cancel tref) true)
          "cancel must accept tref even if tref's timer has already fired")))
  (proc-util/execute-proc!!
    (process/flag :trap-exit true)
    (let [pid (process/spawn-link (process/proc-fn [] (process/receive! _ :ok)))
      ...
      (process/receive!
        [:EXIT _ :killed] :ok
        (after 20 (is false "kill must be sent just after timeout")))
      (is (do (timer/cancel tref) true)
          "cancel must accept tref even if tref's timer has already fired"))))
...
(deftest ^:parallel exit-after.exit-not-existing-process
  ...
  (proc-util/execute-proc!!
    (process/flag :trap-exit true)
    (let [pid (process/spawn (process/proc-fn [] :ok))
          _ (timer/exit-after 20 pid :test1)
          start (System/nanoTime)]
      (timer/exit-after 50 :test2)
      (process/receive!
        [:EXIT _ :test2] (is (>= (ms-diff start) 50)
...
(deftest ^:parallel apply-interval.stops-on-linked-process-exit
  (proc-util/execute-proc!!
    (let [parent (process/self)
          pfn (process/proc-fn []
                (timer/apply-interval 50 #(! parent :msg))
                (process/receive!
                  _ :ok))
          pid (process/spawn pfn)]
      (dotimes [n 3]
